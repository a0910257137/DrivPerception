...
def ap_per_class(tp,
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_l == 0:
            ...
            r[ci] = np.interp(
                -px, -conf[i], recall[:, 0],
                left=0)  # r at pr_score, negative x, xp because xp decreases
    ...
    def plot(self, save_dir='', names=()):
        try:
            ...
            sn.heatmap(array,
                       ...
                       yticklabels=names +
                       ['background FP'] if labels else "auto").set_facecolor(
                           (1, 1, 1))
...
class SegmentationMetric(object):
    ...
    def lineAccuracy(self):
        Acc = np.diag(
            self.confusionMatrix) / (self.confusionMatrix.sum(axis=1) + 1e-12)
        return Acc[1]
    ...
    def classPixelAccuracy(self):
        ...
        # acc = (TP) / TP + FP
        classAcc = np.diag(
            self.confusionMatrix) / (self.confusionMatrix.sum(axis=0) + 1e-12)
        return classAcc
    ...
    def meanIntersectionOverUnion(self):
        ...
        union = np.sum(self.confusionMatrix, axis=1) + np.sum(
            self.confusionMatrix, axis=0) - np.diag(self.confusionMatrix)
    ...
    def Frequency_Weighted_Intersection_over_Union(self):
        ...
        freq = np.sum(self.confusionMatrix, axis=1) / np.sum(
            self.confusionMatrix)
        iu = np.diag(
            self.confusionMatrix) / (np.sum(self.confusionMatrix, axis=1) +
